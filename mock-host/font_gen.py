...
def get_bit_ls(num, n):
    return (num >> n) & 1

def get_bit_ms(num, n):
    return (num << n) & 0x80

def set_bit_ms(num, n):
    return num | (0x80 >> n)

def parse_upy(raw):
    return [int(x, base=0) for x in raw.split(",")]

def upy_to_matrix(upy):
    assert len(upy) == 8
    matrix = [[False] * 8 for _ in range(8)]
    for x in range(8):
        for y in range(8):
            matrix[y][x] = bool(get_bit_ls(upy[x], y))
    return matrix

def matrix_to_lp(matrix):
    ret = []
    for row in matrix:
        byte = 0
        for i, bit in enumerate(row):
            if bit:
                byte = set_bit_ms(byte, i)
        ret.append(byte)
    return ret

def format_lp(lp):
    assert len(lp) == 8
    inner = ',\n        '.join(f"0b{byte:08b}" for byte in lp)
    return """\
    {{
        {}
    }},
...
with open("font_gen_output.txt", "w") as outf:
    for ascii, raw in enumerate(orig.splitlines(), start=32):
        if chr(ascii).isprintable():
            outf.write(f"    // {chr(ascii)}\n")
        outf.write(format_lp(matrix_to_lp(upy_to_matrix(parse_upy(raw)))))